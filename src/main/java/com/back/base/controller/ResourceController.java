...
public class ResourceController extends BaseController {
    ...
    public String list(EResource resource, ModelMap model, HttpServletRequest request) {

        PageContext page = PageContext.getContext(request,rowPerPage);//获得分页标签

        page.setPagination(true);//修改分页状态  是否分页

        List<EResource> list = resourceService.list(resource);
        model.put("list", list);
    ...
    public String saveOrUpdate(EResource resource,ModelMap model) {
